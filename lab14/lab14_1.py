import os
from lab14_functions import print_lists, normalize_path, safe_join
...
def choose(path): #навигация по директориям
    ...
    if os.path.isdir(path):
        ...
        if len(files_list) == 0 and len(dirs_list) == 0:
            print("Ошибка: в директории нет .bin файлов и поддиректорий")
            return None

        if len(files_list) > 0:
            print(print_lists(files_list, "bin файлы:\n"))
        if len(dirs_list) > 0:
