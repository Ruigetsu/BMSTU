...
def find_zeros(f, max_iterations = 100):    
    interval = [-20, 20]
    step = 2  # Уменьшил шаг для более точного поиска
    epsilon = 1e-7
    ...
    while x < b:
        if f(x) * f(x + step) <= 0: 
            zero = bisection(f, x, x + step, epsilon, max_iterations)
...
if all_zeros != False:  
    for i in range(len(all_zeros) - 1):
        ...
        print(f"Площадь образованной графиками замкнутой фигуры = {abs(s1-s2)}")
else: 
...
x_start = -5
x_end = 5
steps = 20  # Количество точек (строк) по вертикали
...
# --- 2. Вычисление масштаба для обоих графиков ---
# Находим общий минимум и максимум среди обоих функций, чтобы они влезли в один масштаб
all_y = y1_vals + y2_vals
min_y2 = min(all_y)
max_y2 = max(all_y)

# Небольшие отступы, чтобы граничные значения не вылетали за рамки
if min_y2 == max_y2: # Защита от деления на ноль, если график плоский
    max_y2 += 1
    min_y2 -= 1

ticks = 10 # Количество засечек на оси Y

# --- Исходный код с модификациями ---
offset = 5 
width = 100 - offset 
...
while j < ticks:
    ...
    # Ограничиваем pos, чтобы не выйти за пределы массива
    if pos >= width + offset: 
        pos = width + offset - 1
        
    if j > 0 and tick_labels and tick_value * float(tick_labels[-1]) < 0: 
        # Добавляем 0, если переходим через него
    ...
    tick_labels.append(f"{tick_value:.1f}") # Сократил до .1f для компактности
    j += 1
...
for k in range(len(tick_positions)):
    if tick_labels[k] != "0":
        ...
        label = tick_labels[k] 
        start = pos
        # Корректировка, если текст вылезает за край
...
print(" "*(offset-1) + "".join(line)) # Добавил отступ для выравнивания с X

for i in range(len(x_ticks_vals)):
    x = x_ticks_vals[i]
    
    # Получаем значения обеих функций
    val1 = y1_vals[i]
    val2 = y2_vals[i]
